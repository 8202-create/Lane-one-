...
function generatePlanFromDescription(text) {
  ...
  DAYS.forEach((day, i) => {
    if (slots[i]) {
      ...
      seqIndex++;
      newPlan[day] = [{ id: Date.now() + i, type, exercises: EXERCISES_BY_TYPE[type].map((name, idx) => ({ id: Date.now() + i * 100 + idx, name, done: false })) }];
    } else {
      newPlan[day] = [];
    }
  });
...
document.getElementById("generateBtn").addEventListener("click", () => {
  ...
  if (!text) return;
  const hasExisting = Object.values(plan).some(sessions => sessions && sessions.length);
  if (hasExisting && !confirm("This replaces your current week's plan. Continue?")) return;
