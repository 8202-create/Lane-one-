...
const MUSCLE_KEYWORDS = {
  ...
  calves: "Calves",
  legs: "Legs", leg: "Legs", quads: "Legs", hamstrings: "Legs",
};
...
function wantsBodyweight(text) {
  return /no equipment|bodyweight|body weight|home workout|without (a )?gym|no gym|no weights?|without weights?/i.test(text);
}
...
// ---------- Explicit day-by-day schedule extraction ----------
// Handles prompts like: "chest day on monday and friday - Pec Fly - Incline DB
// press ... " followed by more days, so a user's own exact routine is used
// verbatim instead of being forced into a generic template.

const DAY_NAME_REGEX = /\b(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/gi;
const DAY_CANON = { monday: "Monday", tuesday: "Tuesday", wednesday: "Wednesday", thursday: "Thursday", friday: "Friday", saturday: "Saturday", sunday: "Sunday" };

function extractListBlock(segment) {
  const paragraphs = segment.split(/\n\s*\n/).map(s => s.trim()).filter(Boolean);
  const listPara = paragraphs.find(p => (p.match(/ - /g) || []).length >= 2 || p.split(",").length >= 3);
  return listPara || "";
}

function extractExplicitDaySchedule(text) {
  const matches = [...text.matchAll(DAY_NAME_REGEX)];
  if (matches.length < 2) return null; // need at least two day mentions to treat this as a full custom schedule

  // Group day names mentioned close together (e.g. "monday and friday", "tuesday n saturday")
  // into one cluster sharing the same exercise list.
  const clusters = [];
  let current = null;
  matches.forEach(m => {
    const day = DAY_CANON[m[0].toLowerCase()];
    if (current && (m.index - current.end) <= 15) {
      current.days.push(day);
      current.end = m.index + m[0].length;
    } else {
      current = { days: [day], start: m.index, end: m.index + m[0].length };
  ...
  clusters.forEach((cluster, i) => {
    const segStart = cluster.end;
    const segEnd = i + 1 < clusters.length ? clusters[i + 1].start : text.length;
    const segment = text.slice(segStart, segEnd);

    const nearbyText = (text.slice(Math.max(0, cluster.start - 20), cluster.start) + segment.slice(0, 30)).toLowerCase();
    if (/\brest\b/.test(nearbyText)) {
      cluster.days.forEach(d => schedule[d] = { type: "Rest", exercises: [] });
      return;
    }

    const listBlock = extractListBlock(segment).replace(/^-\s*/, "");
    const items = listBlock
      .split(/\s-\s/)
      .map(s => s.trim())
      .filter(s => s.length > 1 && s.length < 60);

    const context = (text.slice(Math.max(0, cluster.start - 60), cluster.start) + " " + segment.slice(0, 60)).toLowerCase();
    let label = "Custom";
    for (const [kw, muscleType] of Object.entries(MUSCLE_KEYWORDS)) {
      if (context.includes(kw)) { label = muscleType; break; }
    }

    cluster.days.forEach(d => schedule[d] = { type: label, exercises: items });
  });

  // Any day never mentioned defaults to rest.
  DAYS.forEach(d => { if (!schedule[d]) schedule[d] = { type: "Rest", exercises: [] }; });

  const daysWithExercises = Object.values(schedule).filter(e => e.exercises.length).length;
  return daysWithExercises > 0 ? schedule : null;
}
...
function generatePlanFromDescription(text, age) {
  ...
  // If the user spelled out their own day-by-day exercise list, use it exactly
  // as written rather than forcing it into a generic template.
  const explicitSchedule = extractExplicitDaySchedule(text);
  if (explicitSchedule) {
    const newPlan = {};
    let trainingDays = 0;
    DAYS.forEach(day => {
      const entry = explicitSchedule[day];
      if (!entry.exercises.length) {
        ...
        newPlan[day] = [{
          id: nextId(),
          type: entry.type,
          exercises: entry.exercises.map(name => ({ id: nextId(), name, done: false })),
        }];
      }
    });
    return {
      plan: newPlan,
      summary: { days: trainingDays, focus: "your own exercise list, as written", bodyweight: false, custom: true },
    };
  }
