...
const BADGE_DEFINITIONS = [
  { id: "5k", name: "5K distance", test: r => r.distanceKm >= 5 },
  { id: "10k", name: "10K distance", test: r => r.distanceKm >= 10 },
  { id: "half", name: "Half marathon distance", test: r => r.distanceKm >= 21.0975 },
  { id: "marathon", name: "Marathon distance", test: r => r.distanceKm >= 42.195 },
  { id: "sub5", name: "Sub 5:00 pace", test: r => r.secPerKm < 300 && r.distanceKm >= 1 },
...
function analyzeRun({ distance, unit, hours, minutes, seconds, weight }) {
  ...
  const weightProvided = !!(weight && weight > 0);
  const weightKg = weightProvided ? weight : 70;
  const met = metFromSpeedKph(speedKph);
  const calories = Math.round(met * weightKg * (totalSeconds / 3600));

  return { distanceKm, distanceMi: distanceKm / 1.60934, totalSeconds, secPerKm, secPerMi, speedKph, speedMph, calories, weightKg, weightProvided };
}
...
function buildComparison(r) {
  ...
  line += `That's about ${boltPct}% of Usain Bolt's average speed during his 100m world record (not his peak top speed, which is higher).`;
  return line;
}
...
function drawShareCard(r, unit) {
  ...
  const speedStr = unit === "mi" ? `${r.speedMph.toFixed(1)} mph` : `${r.speedKph.toFixed(1)} km/h`;
  const calStr = r.weightProvided ? `${r.calories} cal` : "cal: add weight to estimate";
  ctx.fillStyle = isLight ? "#171b24" : "#f2f0ea";
  ctx.font = "500 30px Inter, sans-serif";
  ctx.fillText(`${dist}  •  ${speedStr}  •  ${calStr}`, 60, 360);
...
function parseSplits(text) {
  if (!text || !text.trim()) return { splits: [], invalidCount: 0 };
  const parts = text.split(",").map(s => s.trim()).filter(Boolean);
  const splits = [];
  let invalidCount = 0;
  parts.forEach(part => {
    const mmss = part.match(/^(\d{1,3}):([0-5]?\d)$/);
    if (mmss) {
      splits.push(parseInt(mmss[1]) * 60 + parseInt(mmss[2]));
      return;
    }
    const plainNum = Number(part);
    if (part !== "" && isFinite(plainNum) && plainNum > 0) {
      splits.push(plainNum);
      return;
    }
    invalidCount++; // e.g. "abc", "4:99", "-2:30", "5:" — rejected rather than silently coerced
  });
  return { splits, invalidCount };
}
...
// ---------- Step length & pacing consistency (rough estimates) ----------

function estimateStrideLength(speedKph, cadence) {
  if (!cadence) return null;
  const speedMPerMin = (speedKph * 1000) / 60;
  return speedMPerMin / cadence; // meters per step, approximate
}

// Note: this describes split-to-split pacing consistency, not true running
// economy/efficiency (which would require oxygen use, biomechanics, etc.).
function pacingConsistencyLabel(splitStatsResult) {
  return splitStatsResult ? splitStatsResult.consistency : null;
}
...
function buildSmartAnalysis(run) {
  ...
  let rating;
  if (!stats) {
    rating = "First run logged";
  } else if (pastRuns.length < 3) {
    // Too small a sample to call it "excellent" or "needs improvement" —
    // just state the objective comparison instead of judging it.
    const diff = run.secPerKm - stats.avgPaceSecPerKm;
    rating = diff < -3 ? "Faster than your previous run"
      : diff > 3 ? "Slower than your previous run"
      : "About the same as your previous run";
  } else {
    const avgPace = stats.avgPaceSecPerKm;
    if (run.secPerKm <= avgPace * 0.95) rating = "Faster than your recent average";
    else if (run.secPerKm <= avgPace * 1.05) rating = "About your recent average";
    else rating = "Slower than your recent average";
  }
...
document.getElementById("runForm").addEventListener("submit", (e) => {
  ...
  if (!isFinite(distance) || distance <= 0) {
    errorEl.textContent = "Enter a distance greater than zero.";
    errorEl.hidden = false;
    return;
  }
  if (distance > 500) {
    errorEl.textContent = "That distance looks unusually large — double check it before continuing.";
    errorEl.hidden = false;
    return;
  }
  const totalInputSeconds = hours * 3600 + minutes * 60 + seconds;
  if (totalInputSeconds <= 0) {
    errorEl.textContent = "Enter a time greater than zero.";
    errorEl.hidden = false;
    return;
  }
  if (hours > 24) {
    errorEl.textContent = "That's more than 24 hours — double check the time before continuing.";
    errorEl.hidden = false;
    return;
  }
  errorEl.hidden = true;
  ...
  const { splits: splitsSec, invalidCount: invalidSplitCount } = parseSplits(document.getElementById("splits").value);
  const sStats = splitStats(splitsSec);
  const strideLength = estimateStrideLength(r.speedKph, cadence);
  const efficiency = pacingConsistencyLabel(sStats);
  ...
  document.getElementById("statSpeed").textContent = unit === "mi" ? r.speedMph.toFixed(1) : r.speedKph.toFixed(1);
  document.getElementById("statSpeedLabel").textContent = unit === "mi" ? "mph average speed" : "km/h average speed";
  if (r.weightProvided) {
    document.getElementById("statCalories").textContent = r.calories;
    document.getElementById("statCaloriesLabel").textContent = "calories, estimated";
  } else {
    document.getElementById("statCalories").textContent = "—";
    document.getElementById("statCaloriesLabel").textContent = "add your weight to estimate calories";
  }

  if (r.distanceKm <= 5) {
    document.getElementById("statDistancePct").textContent = `${Math.round((r.distanceKm / 5) * 100)}%`;
    document.getElementById("statDistanceLabel").textContent = "of a 5K";
  } else {
    document.getElementById("statDistancePct").textContent = `${(r.distanceKm / 5).toFixed(1)}×`;
    document.getElementById("statDistanceLabel").textContent = "a 5K distance";
  }
  ...
  if (strideLength) advancedEntries.push([strideLength.toFixed(2) + " m", "est. step length"]);
  if (efficiency) advancedEntries.push([efficiency, "pacing consistency"]);
  ...
  if (sStats) {
    ...
    document.querySelector("#splitsRow canvas").hidden = false;
    ...
    document.getElementById("splitsSummary").textContent =
      ...
      + (invalidSplitCount > 0 ? ` (${invalidSplitCount} entr${invalidSplitCount > 1 ? "ies" : "y"} couldn't be read and ${invalidSplitCount > 1 ? "were" : "was"} skipped — use mm:ss format like 4:32.)` : "");
  } else if (invalidSplitCount > 0) {
    splitsRow.hidden = false;
    document.querySelector("#splitsRow canvas").hidden = true;
    document.getElementById("splitsSummary").textContent =
      `None of the splits entered could be read. Use mm:ss format separated by commas, e.g. 4:32, 4:41, 4:38.`;
  } else {
...
document.getElementById("fileUpload").addEventListener("change", (e) => {
  ...
  reader.onerror = () => {
    uploadNote.textContent = "We couldn't read this file. Make sure it's a valid GPX or TCX activity export.";
  };
  ...
  reader.onload = (evt) => {
    try {
      ...
      if (!summary.distanceKm || !isFinite(summary.distanceKm) || summary.totalSeconds <= 0) {
        uploadNote.textContent = "We couldn't read this file. Make sure it's a valid GPX or TCX activity export with track points and timestamps.";
        return;
      }
      ...
      uploadNote.textContent = `Imported: ${summary.distanceKm.toFixed(2)} km, elevation gain ${summary.elevationGain} m. This uses elapsed time (start to finish), not moving time — pauses will affect the calculated pace.`;
    } catch (err) {
      uploadNote.textContent = "We couldn't read this file. Make sure it's a valid GPX or TCX activity export.";
    }
  };
