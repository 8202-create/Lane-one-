...
const STORAGE_KEYS = {
  ...
  BODY_LOG: "laneone_body_log",
};

function getBodyLog() {
  try { return JSON.parse(localStorage.getItem(STORAGE_KEYS.BODY_LOG)) || []; }
  catch (e) { return []; }
}

function saveBodyLogList(list) {
  localStorage.setItem(STORAGE_KEYS.BODY_LOG, JSON.stringify(list));
}

function logBodyEntry(entry) {
  const list = getBodyLog();
  entry.id = Date.now();
  entry.date = entry.date || new Date().toISOString();
  list.push(entry);
  list.sort((a, b) => new Date(a.date) - new Date(b.date));
  saveBodyLogList(list);
  return entry;
}

function deleteBodyEntry(id) {
  saveBodyLogList(getBodyLog().filter(e => e.id !== id));
}
