...
document.getElementById("bodyDate").value = new Date().toISOString().split("T")[0];
renderBodyLog();

document.getElementById("bodyLogForm").addEventListener("submit", (e) => {
  e.preventDefault();
  const weight = parseFloat(document.getElementById("bodyWeight").value);
  if (!weight || weight <= 0) return;
  const date = document.getElementById("bodyDate").value || new Date().toISOString().split("T")[0];
  const waist = parseFloat(document.getElementById("bodyWaist").value) || null;
  const arms = parseFloat(document.getElementById("bodyArms").value) || null;

  logBodyEntry({ date: new Date(date).toISOString(), weight, waist, arms });
...
function renderBodyLog() {
  ...
  document.getElementById("bodyLogEmpty").hidden = log.length > 0;
  document.getElementById("bodyLogChartWrap").hidden = log.length < 2;

  if (log.length >= 2) {
    const points = log.map((e, i) => ({ x: i + 1, y: e.weight }));
    drawLineChart("bodyWeightChart", points, { color: "#ff8a3d", yLabel: "weight (kg)" });
  }

  const list = document.getElementById("bodyLogList");
  const recent = [...log].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 10);
  list.innerHTML = recent.map(e => `
    <div class="history-row">
      <span class="history-date">${new Date(e.date).toLocaleDateString()}</span>
      <span class="history-type">${e.weight} kg${e.waist ? ` · waist ${e.waist}cm` : ""}${e.arms ? ` · arms ${e.arms}cm` : ""}</span>
      <button class="text-link history-delete-btn" data-id="${e.id}">Delete</button>
    </div>
  `).join("");

  list.querySelectorAll(".history-delete-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      deleteBodyEntry(Number(btn.dataset.id));
      renderBodyLog();
    });
  });
}
